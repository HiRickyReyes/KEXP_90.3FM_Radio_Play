...
# load logo
logo_filepath = "dataset_assets/kexp_logo.png"
...
with col2:
    ...
    top_hosts_df.index = range(1, len(top_hosts_df) + 1)
    # center-align text in the dataframe but left-align headers
    st.dataframe(
...
# Chart 1: Stacked Bar Chart for Hosts Introducing New Artists
fig2 = px.bar(
    yearly_counts,
    x="Year",
    y="New Artists",
...
st.markdown(
    f"""
    **Sample:** This analysis is based on a 1% random sample of the full KEXP playlist dataset, which comprises 2,667,360 records. The sample ensures computational efficiency while maintaining the integrity and representativeness of the original data.
    """,
    unsafe_allow_html=True,
)
